...
class PollActiveTests(unittest.TestCase):
    ...
    def test_closed_saves_voters(self):
        ...
        poll.poll_plugin = 'majority_poll'
        poll.workflow.do_transition('ongoing', force=True)
        poll.workflow.do_transition('closed', force=True)
        self.assertEqual(poll.voters_mark_closed, frozenset(['alice', 'ben', 'celine']))
    ...
    def test_ongoing_saves_voters(self):
        ...
        self.assertEqual(poll.voters_mark_ongoing, frozenset(['alice', 'ben', 'celine', 'admin']))
    ...
    def test_poll_is_deleted(self):
        ...
        ai = meeting['ai']
        ai['poll'].wf_state = 'ongoing'
        # making sure that proposals are in voting state after poll is set to ongoing
        self.assertEqual(ai['prop1'].wf_state, 'voting')
        self.assertEqual(ai['prop2'].wf_state, 'voting')
        self.config.include('voteit.core.subscribers.poll')
        del ai['poll']
        self.assertEqual(ai['prop1'].wf_state, 'published')
        self.assertEqual(ai['prop2'].wf_state, 'published')
