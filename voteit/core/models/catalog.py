...
def update_contained_in_ai(obj, event):
    ...
    """
    if event.from_state == 'private' or event.to_state == 'private':
...
def includeme(config):
    ...
    config.add_subscriber(update_contained_in_ai, [IAgendaItem, IWorkflowAfterTransition])
    config.update_index_info('aid_int', linked='aid_int', type_names = 'Proposal')
    config.update_index_info('aid', type_names = 'Proposal')
    config.update_index_info('allowed_to_view', linked='wf_state')
